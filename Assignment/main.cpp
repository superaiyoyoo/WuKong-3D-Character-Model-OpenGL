...
float upperBodyRotationY = 0.0f;      // Upper body Y-axis rotation
bool isUpperBodyRotating = false;      // Toggle for upper body rotation
int upperBodyRotationDir = 1;          // Direction: 1 = right, -1 = left
bool wasOPressed = false;

// Sound effect variables
bool bombSoundPlayed = false;           // Track if bomb sound has been played for current ice cream session
bool lastIceCreamState = false;        // Track previous ice cream visibility state
...
// Time-based animation variables to prevent fast refresh issues
DWORD lastFlowerUpdateTime = 0;        // Last time flower animation was updated
const float flowerAnimationDelta = 16.67f; // Target 60 FPS (16.67ms per frame)

float timeFactor = 1.0f;  // Time multiplier for frame-rate independent animation

// Forest texture variables
GLuint forestTexture = 0;              // Forest texture ID
bool forestTextureEnabled = false;     // Toggle for forest texture

GLuint hillTexture = 0;                // Hill texture ID
bool hillTextureEnabled = false;
...
// Function to play loud bomb explosion sound
void playBombExplosionSound() {
    // Try multiple bomb explosion sound effects for maximum impact
    // You can use any of these - the function will try them in order

    // Method 1: Play from file (if you have a bomb.wav file)
    if (PlaySound(TEXT("bomb.wav"), NULL, SND_FILENAME | SND_ASYNC | SND_NODEFAULT)) {
        return; // Successfully played custom bomb sound
    }

    // Method 2: Play system explosion sound (if available)
    if (PlaySound(TEXT("SystemExclamation"), NULL, SND_ALIAS | SND_ASYNC | SND_NODEFAULT)) {
        return; // Successfully played system sound
    }

    // Method 3: Create loud beep sequence for explosion effect
    // Multiple rapid beeps to simulate explosion
    for (int i = 0; i < 5; i++) {
        Beep(200 - (i * 20), 100);  // Decreasing frequency, short duration
        Sleep(50);                   // Short pause between beeps
    }

    // Method 4: Additional system beeps for maximum loudness
    Beep(150, 200);   // Low rumble
    Beep(800, 150);   // High explosion peak
    Beep(400, 200);   // Mid explosion
    Beep(100, 300);   // Deep explosion end
}

// Enhanced function for MAXIMUM LOUD bomb explosion
void playLoudBombExplosion() {
    // Start background system sound
    PlaySound(TEXT("SystemExclamation"), NULL, SND_ALIAS | SND_ASYNC | SND_NODEFAULT);

    // Create a thread for continuous loud beeping
    CreateThread(NULL, 0, [](LPVOID) -> DWORD {
        // Rapid fire explosion sequence
        for (int i = 0; i < 8; i++) {
            Beep(1000 - (i * 100), 80); // Rapid decreasing beeps
            Sleep(30);
        }
        ...
        // Final explosion peak
        Beep(1500, 300);

        return 0;
        }, NULL, 0, NULL);

    // Additional immediate beeps for instant impact
    Beep(2000, 100); // Immediate high-pitched explosion start
}
...
void renderFlowerFieldBackground()
{
    ...
    // Ground with forest texture or default gradient
    if (forestTextureEnabled && forestTexture != 0) {
        // Enable texture mapping for forest ground
        glEnable(GL_TEXTURE_2D);
        glBindTexture(GL_TEXTURE_2D, forestTexture);
        glColor3f(1.0f, 1.0f, 1.0f);  // White to show true texture colors

        glBegin(GL_QUADS);
        // Apply forest texture to the entire ground area
        glTexCoord2f(0.0f, 0.0f); glVertex2f(-1.0f, -1.0f);
        glTexCoord2f(3.0f, 0.0f); glVertex2f(1.0f, -1.0f);     // Repeat texture 3x horizontally
        glTexCoord2f(3.0f, 2.0f); glVertex2f(1.0f, -0.3f);     // Repeat texture 2x vertically
        glTexCoord2f(0.0f, 2.0f); glVertex2f(-1.0f, -0.3f);
        glEnd();

        glDisable(GL_TEXTURE_2D);
    }
    else {
        // Default gradient grass background (your existing code)
        glBegin(GL_QUADS);
    ...
    // Hills with hill texture or default gradient - KEEPING ORIGINAL COORDINATES
    if (hillTextureEnabled && hillTexture != 0) {
        // Enable texture mapping for hills
        glEnable(GL_TEXTURE_2D);
        glBindTexture(GL_TEXTURE_2D, hillTexture);
        glColor3f(1.0f, 1.0f, 1.0f);  // White to show true texture colors

        // First row of hills with texture - SAME EXACT COORDINATES
        glBegin(GL_POLYGON);
        glTexCoord2f(0.0f, 0.0f); glVertex2f(-1.0f, -0.3f);
        glTexCoord2f(0.1f, 0.2f); glVertex2f(-0.8f, -0.25f);
        glTexCoord2f(0.2f, 0.1f); glVertex2f(-0.6f, -0.28f);
        glTexCoord2f(0.3f, 0.3f); glVertex2f(-0.4f, -0.22f);
        glTexCoord2f(0.4f, 0.15f); glVertex2f(-0.2f, -0.27f);
        glTexCoord2f(0.5f, 0.35f); glVertex2f(0.0f, -0.23f);
        glTexCoord2f(0.6f, 0.1f); glVertex2f(0.2f, -0.28f);
        glTexCoord2f(0.7f, 0.25f); glVertex2f(0.4f, -0.24f);
        glTexCoord2f(0.8f, 0.05f); glVertex2f(0.6f, -0.29f);
        glTexCoord2f(0.9f, 0.2f); glVertex2f(0.8f, -0.26f);
        glTexCoord2f(1.0f, 0.0f); glVertex2f(1.0f, -0.3f);
        glEnd();

        // Second row of more distant hills with texture - SAME EXACT COORDINATES
        glBegin(GL_POLYGON);
        glTexCoord2f(0.0f, 0.5f); glVertex2f(-1.0f, -0.3f);
        glTexCoord2f(0.1f, 0.65f); glVertex2f(-0.9f, -0.27f);
        glTexCoord2f(0.2f, 0.7f); glVertex2f(-0.7f, -0.25f);
        glTexCoord2f(0.3f, 0.6f); glVertex2f(-0.5f, -0.28f);
        glTexCoord2f(0.4f, 0.75f); glVertex2f(-0.3f, -0.24f);
        glTexCoord2f(0.5f, 0.8f); glVertex2f(-0.1f, -0.26f);
        glTexCoord2f(0.6f, 0.9f); glVertex2f(0.1f, -0.23f);
        glTexCoord2f(0.7f, 0.7f); glVertex2f(0.3f, -0.25f);
        glTexCoord2f(0.8f, 0.85f); glVertex2f(0.5f, -0.22f);
        glTexCoord2f(0.9f, 0.6f); glVertex2f(0.7f, -0.26f);
        glTexCoord2f(1.0f, 0.75f); glVertex2f(0.9f, -0.24f);
        ...
        glBegin(GL_POLYGON);
        glColor3f(forestTextureEnabled ? 0.2f : 0.4f, forestTextureEnabled ? 0.4f : 0.7f, forestTextureEnabled ? 0.2f : 0.4f);  // Darker if forest texture is enabled
        glVertex2f(-1.0f, -0.3f);
        ...
        glBegin(GL_POLYGON);
        glColor3f(forestTextureEnabled ? 0.15f : 0.3f, forestTextureEnabled ? 0.35f : 0.6f, forestTextureEnabled ? 0.18f : 0.35f);  // Even darker if forest texture is enabled
        glVertex2f(-1.0f, -0.3f);
    ...
    // Time-based flower animation updates to prevent fast refresh during texture loading
    DWORD currentTime = GetTickCount();
    if (lastFlowerUpdateTime == 0) {
        lastFlowerUpdateTime = currentTime;
    }

    float deltaTime = (currentTime - lastFlowerUpdateTime);
    if (deltaTime >= flowerAnimationDelta) {
        // Update rainbow mode colors if active - time-based
        if (rainbowMode && deltaTime >= flowerAnimationDelta) {
            rainbowColorCycle += 0.01f;
            if (rainbowColorCycle > 6.28f) rainbowColorCycle = 0.0f;
        }

        // Update flower sway animation with time-based calculation
        flowerSway += flowerSwaySpeed;
        if (flowerSway > 6.28f) flowerSway = 0.0f;

        // Update flower count cycle with time-based calculation
...
// Helper function to draw sprinkles
void drawSprinkles(float centerX, float centerY, float centerZ, float radius, int count)
{
    // Make sprinkles much larger and more visible
    glPointSize(4.0f);

    // Draw sprinkles as small cylinders for better visibility
    GLUquadricObj* sprinkleQuad = gluNewQuadric();
    gluQuadricDrawStyle(sprinkleQuad, GLU_FILL);

    for (int i = 0; i < count; i++) {
        // Generate random positions on the sphere surface
        float theta = (float)(rand() % 360) * 3.14159f / 180.0f;
        float phi = (float)(rand() % 180) * 3.14159f / 180.0f;

        float x = centerX + radius * sin(phi) * cos(theta) * 0.95f; // Close to surface
        float y = centerY + radius * cos(phi) * 0.95f;
        float z = centerZ + radius * sin(phi) * sin(theta) * 0.95f;

        // Random bright colors for sprinkles
        switch (i % 8) {
        case 0: glColor3f(1.0f, 0.0f, 1.0f); break; // Magenta
        case 1: glColor3f(0.0f, 1.0f, 1.0f); break; // Cyan
        case 2: glColor3f(1.0f, 1.0f, 0.0f); break; // Yellow
        case 3: glColor3f(1.0f, 0.5f, 0.0f); break; // Orange
        case 4: glColor3f(0.5f, 1.0f, 0.0f); break; // Lime green
        case 5: glColor3f(1.0f, 0.0f, 0.5f); break; // Hot pink
        case 6: glColor3f(0.0f, 0.5f, 1.0f); break; // Blue
        case 7: glColor3f(1.0f, 0.2f, 0.8f); break; // Pink
        }

        // Draw each sprinkle as a small cylinder
        glPushMatrix();
        glTranslatef(x, y, z);
        // Random rotation for more realistic look
        glRotatef((float)(rand() % 360), 1.0f, 0.0f, 0.0f);
        glRotatef((float)(rand() % 360), 0.0f, 1.0f, 0.0f);
        gluCylinder(sprinkleQuad, 0.008f, 0.008f, 0.025f, 6, 1); // Small cylinder sprinkles
        glPopMatrix();
    }

    gluDeleteQuadric(sprinkleQuad);
}

// Helper function to draw concentrated sprinkles on top hemisphere (more visible)
void drawTopSprinkles(float centerX, float centerY, float centerZ, float radius, int count)
{
    glPointSize(5.0f);
    ...
    for (int i = 0; i < count; i++) {
        // Focus sprinkles on the top hemisphere (more visible area)
        float theta = (float)(rand() % 360) * 3.14159f / 180.0f;
        float phi = (float)(rand() % 90 + 45) * 3.14159f / 180.0f; // Top hemisphere only

        float x = centerX + radius * sin(phi) * cos(theta) * 0.98f;
        float y = centerY + radius * cos(phi) * 0.98f;
        float z = centerZ + radius * sin(phi) * sin(theta) * 0.98f;

        // Extra bright colors for top sprinkles
        switch (i % 10) {
        case 0: glColor3f(1.0f, 0.0f, 1.0f); break; // Bright magenta
        case 1: glColor3f(0.0f, 1.0f, 1.0f); break; // Bright cyan
        case 2: glColor3f(1.0f, 1.0f, 0.0f); break; // Bright yellow
        case 3: glColor3f(1.0f, 0.3f, 0.0f); break; // Bright orange
        case 4: glColor3f(0.3f, 1.0f, 0.0f); break; // Bright lime
        case 5: glColor3f(1.0f, 0.0f, 0.3f); break; // Bright pink
        case 6: glColor3f(0.0f, 0.3f, 1.0f); break; // Bright blue
        case 7: glColor3f(1.0f, 0.1f, 0.9f); break; // Hot pink
        case 8: glColor3f(0.8f, 0.0f, 1.0f); break; // Purple
        case 9: glColor3f(1.0f, 0.8f, 0.0f); break; // Gold
        }

        // Draw larger sprinkles on top
        glPushMatrix();
        glTranslatef(x, y, z);
        glRotatef((float)(rand() % 360), 1.0f, 0.0f, 0.0f);
        glRotatef((float)(rand() % 360), 0.0f, 1.0f, 0.0f);
        gluCylinder(sprinkleQuad, 0.012f, 0.012f, 0.035f, 8, 1); // Bigger sprinkles on top
        glPopMatrix();
    }
...
// Helper function to draw gradient sphere
void drawGradientSphere(GLUquadricObj* quadric, float radius, float r1, float g1, float b1, float r2, float g2, float b2, int slices, int stacks)
{
    ...
    for (int i = 0; i < stacks; i++) {
        ...
        // Calculate gradient factor based on height
        float factor = (float)i / (float)stacks;
...
void drawIceCreamCone()
{
    ...
    // Set camera position - move closer to see the smaller ice cream
    glTranslatef(0.0f, 0.0f, -2.5f);
    glRotatef(g_rotation_y, 0.0f, 0.01f, 0.0f);

    // --- Draw the wafer cone ---
    GLUquadricObj* pObj = gluNewQuadric();
    glPushMatrix();
    {
        // Move the cone into position first
        glTranslatef(0.0f, -0.05f, 0.0f); // Reduced Y position
        glRotatef(90.0f, 0.1f, 0.0f, 0.0f);

        // **1. Enable and set the polygon offset to prevent Z-fighting**
        glEnable(GL_POLYGON_OFFSET_FILL);
        glPolygonOffset(1.0f, 1.0f);

        glColor3f(1.0f, 0.8f, 0.2f); // Bright golden wafer cone color
        gluQuadricDrawStyle(pObj, GLU_FILL);
        gluCylinder(pObj, 0.3, 0.0, 0.8, 20, 10); // INCREASED: radius 0.3, height 0.8

        // **3. Disable the offset**
        glDisable(GL_POLYGON_OFFSET_FILL);

        // **4. Draw the dark brown wireframe on top (SECOND PASS)**
        glColor3f(0.6f, 0.3f, 0.0f); // Dark brown wafer pattern
        gluQuadricDrawStyle(pObj, GLU_LINE);
        gluCylinder(pObj, 0.3, 0.0, 0.8, 20, 10); // INCREASED: radius 0.3, height 0.8
    }
    glPopMatrix();

    gluQuadricDrawStyle(pObj, GLU_FILL); // Fill style for the cone
    ...
    // --- Draw the scoops with gradient colors ---

    // Bottom (Strawberry Pink to Red Gradient) Scoop
    glPushMatrix();
    glTranslatef(0.0f, -0.05f, 0.0f); // Reduced Y position
    drawGradientSphere(pObj, 0.25, 1.0f, 0.4f, 0.6f, 1.0f, 0.1f, 0.3f, 20, 15); // INCREASED: radius 0.25
    glPopMatrix();

    // Top (Sky Blue to Purple Gradient) Scoop
    glPushMatrix();
    glTranslatef(0.0f, 0.15f, 0.0f); // Increased Y position
    drawGradientSphere(pObj, 0.22, 0.3f, 0.8f, 1.0f, 0.6f, 0.2f, 1.0f, 20, 15); // INCREASED: radius 0.22
    glPopMatrix();

    // LOTS of sprinkles on top scoop (much more visible) 
    drawTopSprinkles(0.0f, 0.15f, 0.0f, 0.22f, 25); // INCREASED: radius 0.22, adjusted Y position

    // --- Draw the toppings ---
    // Cherry (Bright Red Sphere)
    glPushMatrix();
    glColor3f(1.0f, 0.1f, 0.2f); // Bright cherry red color
    glTranslatef(0.1f, 0.35f, 0.0f); // INCREASED: X offset 0.1, Y position 0.35
    gluSphere(pObj, 0.05, 20, 10); // INCREASED: radius 0.05
    glPopMatrix();

    // Chocolate Stick (Rich Chocolate Brown Cylinder)
    glPushMatrix();
    glColor3f(0.5f, 0.25f, 0.1f); // Rich chocolate brown color
    glTranslatef(0.0f, 0.3f, 0.0f); // INCREASED: Y position 0.3
    glRotatef(-45.0f, 0.0f, 0.0f, 1.0f);
    gluCylinder(pObj, 0.025, 0.025, 0.35, 10, 5); // INCREASED: radius 0.025, height 0.35
    glPopMatrix();

    // --- NEW TOPPINGS: Mini Candy Spheres ---
    // Small candy 1 (Green)
    glPushMatrix();
    glColor3f(0.2f, 1.0f, 0.3f); // Bright green
    glTranslatef(-0.08f, 0.32f, 0.05f); // INCREASED: all positions doubled
    gluSphere(pObj, 0.03, 10, 8); // INCREASED: radius 0.03
    glPopMatrix();

    // Small candy 2 (Orange)
    glPushMatrix();
    glColor3f(1.0f, 0.6f, 0.0f); // Bright orange
    glTranslatef(0.05f, 0.28f, -0.08f); // INCREASED: all positions doubled
    gluSphere(pObj, 0.025, 10, 8); // INCREASED: radius 0.025
    glPopMatrix();

    // --- Curved Wafer ---
    glPushMatrix();
    glColor3f(1.0f, 0.7f, 0.3f); // Bright golden wafer color
    glLineWidth(2.0f); // REDUCED: thinner line
    glTranslatef(-0.05f, 0.25f, 0.0f); // INCREASED: all positions doubled
    glRotatef(-20.0f, 1.0f, 0.0f, 0.0f);
    glBegin(GL_LINE_STRIP);
    for (int i = 0; i <= 180; i++) {
        float angle = i * 3.14159f / 180.0f;
        float x = 0.2f * cos(angle); // INCREASED: 0.2f
        float y = 0.2f * sin(angle); // INCREASED: 0.2f
        glVertex3f(x, y, 0.0f);
    }
    glEnd();
    glPopMatrix();

    gluDeleteQuadric(pObj); // Free the quadric object's memory [cite: 20, 21]
}
...
// Global variables for ice cream cone toggle
bool showIceCreamCone = false;  // Toggle state for ice cream cone
bool wasFPressed = false;       // Track F key state to prevent rapid toggling
...
float maxSlapAngle = 19.0f;       // Maximum angle for slapping motion
...
// Dance animation system variables
bool isDancing = false;            // Track if dance animation is active
bool wasVPressed = false;          // Track V key state to prevent rapid toggling
float danceAnimationTime = 0.0f;   // Current dance animation time
float danceAnimationDuration = 5.0f; // Total duration of dance animation (3 seconds for more entertainment)
float danceArmAngle[2] = { 0.0f, 0.0f }; // Current dance arm angles for both arms
float danceLegAngle[2] = { 0.0f, 0.0f }; // Current dance leg angles for both legs
float danceBodyAngle = 0.0f;       // Current body rotation angle for dance
int dancePhase = 0;                // 0 = not active, 1 = wobble start, 2 = moonwalk, 3 = robot dance, 4 = grand finale
...
LRESULT WINAPI WindowProcedure(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_KEYDOWN:
        ...
        case '3': // toggle fire texture on weapon
            ...
            break;
        case '4': // toggle forest and hill textures on grass/ground and hills
            forestTextureEnabled = !forestTextureEnabled;
            hillTextureEnabled = !hillTextureEnabled;  // Toggle both together

            if (forestTextureEnabled) {
                // Load forest texture if not already loaded
                if (forestTexture == 0) {
                    forestTexture = loadTexture("forest.bmp");
                }
            }

            if (hillTextureEnabled) {
                // Load hill texture if not already loaded
                if (hillTexture == 0) {
                    hillTexture = loadTexture("hill.bmp");
                }
            }
            break;
        case '5': // toggle sky texture 
            skyTextureEnabled = !skyTextureEnabled;

            if (skyTextureEnabled) {
                // Load sky texture if not already loaded
                if (skyTexture == 0) {
        ...
        case 'O':
            if (!wasOPressed) {
                isUpperBodyRotating = !isUpperBodyRotating;
                wasOPressed = true;
            }
            break;
        ...
        case 'V': // dance animation
            if (!isDancing) {
                // Start dance animation only if not already animating
                isDancing = true;
                danceAnimationTime = 0.0f;  // Reset animation time
                danceBodyAngle = 0.0f;       // Start from neutral position
                dancePhase = 1;              // Start with wobble phase
                // Reset dance angles for arms and legs
                for (int i = 0; i < 2; i++) {
                    danceArmAngle[i] = 0.0f;
                    danceLegAngle[i] = 0.0f;
                }
            }
            break;

        case 'F': // toggle ice cream cone
            if (!wasFPressed) {
                showIceCreamCone = !showIceCreamCone;
                wasFPressed = true;
            }
            break;
        ...
        case VK_SPACE: // reset all elements to default
            ...
            isDancing = false;
            danceAnimationTime = 0.0f;
            danceBodyAngle = 0.0f;
            dancePhase = 0;  // Reset to not active
            for (int i = 0; i < 2; i++) {
                danceArmAngle[i] = 0.0f;
                danceLegAngle[i] = 0.0f;
            }
...
void drawArm(int armIndex)
{
    ...
    // --- Apply manual control, walking swing, smash attack, hi animation, bow animation, slapping animation, or dance animation
    if ((currentControlMode == LEFT_ARM && armIndex == 0) ||
        (currentControlMode == RIGHT_ARM && armIndex == 1)) {
        ...
        // Apply dance animation for both arms
        glRotatef(danceArmAngle[armIndex], 1, 0, 0);  // Both arms move during dance
    }
    else {
    ...
    // Now apply the swing forward/back, smash attack, hi animation, bow animation, slapping, or dance animation
    ...
    //drawFurOnSphere(0.5f, 10);
      ...
      // Apply wrist animation - hi animation, bow animation, slapping animation, dance animation, normal walking, or default
    if (armIndex == 1 && isHiAnimating) {
        ...
        // Apply dance wrist animation for both arms
        glRotatef(danceArmAngle[armIndex] * 0.5f, 0, 1, 0);  // Subtle wrist movement during dance
    }
    else {
        // Normal walking animation
...
void handleInput()
{
    ...
    // Ice cream cone toggle (F key) - prevent rapid toggling
    bool fPressed = (GetAsyncKeyState('F') & 0x8000) != 0;
    if (fPressed && !wasFPressed) {
        showIceCreamCone = !showIceCreamCone;
    }
    wasFPressed = fPressed;

    // Dance animation (V key) - prevent rapid toggling
    bool vPressed = (GetAsyncKeyState('V') & 0x8000) != 0;
    ...
    if (oPressed && !wasOPressed) {
        isUpperBodyRotating = !isUpperBodyRotating;
    }
    wasOPressed = oPressed;

    // Continuous upper body rotation when enabled
    if (isUpperBodyRotating) {
        upperBodyRotationY += upperBodyRotationDir * 1.0f;  // Rotation speed with direction
        // Limit upper body rotation to 90 degrees total range (-45 to +45 degrees)
        // and reverse direction when hitting limits
        if (upperBodyRotationY >= 25.0f) {
            upperBodyRotationY = 25.0f;  // Maximum right rotation
            upperBodyRotationDir = -1;    // Change direction to left
        }
        else if (upperBodyRotationY <= -25.0f) {
            upperBodyRotationY = -25.0f;  // Maximum left rotation
            upperBodyRotationDir = 1;     // Change direction to right
        }
    }
...
void display()
{
    ...
    if (isDancing) {
        // Increment animation time (faster for more energetic dance)
        danceAnimationTime += 0.03f;  // Faster animation for more comedic effect

        // Calculate animation progress (0.0 to 1.0)
        float progress = danceAnimationTime / danceAnimationDuration;
        ...
        if (progress >= 1.0f) {
            ...
            // Multi-phase funny dance animation
            if (progress < 0.15f) {
                // Phase 1: Funny start - wobble and shake (0-15% of animation)
                dancePhase = 1;
                float startProgress = progress / 0.15f;  // 0 to 1 for this phase

                // Exaggerated body wobble
                danceBodyAngle = 25.0f * sinf(startProgress * M_PI * 3.0f);  // Wild side-to-side wobble

                // Crazy arm movements - flailing
                danceArmAngle[0] = 50.0f * sinf(startProgress * M_PI * 5.0f);  // Left arm flailing
                danceArmAngle[1] = 50.0f * cosf(startProgress * M_PI * 4.0f);  // Right arm flailing opposite

                // Leg jitter - nervous energy
                danceLegAngle[0] = 20.0f * sinf(startProgress * M_PI * 8.0f);  // Left leg jitter
                danceLegAngle[1] = 20.0f * cosf(startProgress * M_PI * 6.0f);  // Right leg jitter opposite

            }
            else if (progress < 0.4f) {
                // Phase 2: The Moonwalk parody (15%-40% of animation)
                dancePhase = 2;
                float moonwalkProgress = (progress - 0.15f) / 0.25f;  // 0 to 1 for this phase

                // Smooth body lean back
                danceBodyAngle = 15.0f - (30.0f * moonwalkProgress);  // Lean back then forward

                // Arms in classic moonwalk position
                danceArmAngle[0] = 45.0f * sinf(moonwalkProgress * M_PI);  // Left arm smooth motion
                danceArmAngle[1] = 45.0f * sinf(moonwalkProgress * M_PI + M_PI);  // Right arm opposite

                // Legs doing the moonwalk slide
                danceLegAngle[0] = 30.0f * (1.0f - moonwalkProgress);  // Left leg slides back
                danceLegAngle[1] = -15.0f * moonwalkProgress;  // Right leg moves forward

            }
            else if (progress < 0.7f) {
                // Phase 3: The Robot Dance (40%-70% of animation)
                dancePhase = 3;
                float robotProgress = (progress - 0.4f) / 0.3f;  // 0 to 1 for this phase

                // Stiff body movements
                danceBodyAngle = 20.0f * sinf(robotProgress * M_PI * 2.0f);  // Mechanical body sway

                // Robotic arm movements - stiff and jerky
                if (robotProgress < 0.5f) {
                    danceArmAngle[0] = 60.0f;  // Left arm up and stuck
                    danceArmAngle[1] = -30.0f;  // Right arm down and stuck
                }
                else {
                    danceArmAngle[0] = -30.0f;  // Switch positions
                    danceArmAngle[1] = 60.0f;  // Switch positions
                }

                // Robotic leg movements
                if (robotProgress < 0.33f) {
                    danceLegAngle[0] = 25.0f;  // Left leg up
                    danceLegAngle[1] = 0.0f;  // Right leg neutral
                }
                else if (robotProgress < 0.66f) {
                    danceLegAngle[0] = 0.0f;  // Left leg neutral
                    danceLegAngle[1] = 25.0f;  // Right leg up
                }
                else {
                    danceLegAngle[0] = -15.0f;  // Left leg down
                    danceLegAngle[1] = -15.0f;  // Right leg down
                }

            }
            else {
                // Phase 4: The Grand Finale - Spinning and Jumping (70%-100% of animation)
                dancePhase = 4;
                float finaleProgress = (progress - 0.7f) / 0.3f;  // 0 to 1 for this phase

                // Wild spinning
                danceBodyAngle = 360.0f * finaleProgress;  // Full 360 spin

                // Arms flailing wildly during spin
                danceArmAngle[0] = 80.0f * sinf(finaleProgress * M_PI * 10.0f);  // Left arm wild flail
                danceArmAngle[1] = 80.0f * cosf(finaleProgress * M_PI * 8.0f);  // Right arm wild flail

                // Legs doing the splits jump
                danceLegAngle[0] = 45.0f * sinf(finaleProgress * M_PI * 3.0f);  // Left leg jumping
                danceLegAngle[1] = 45.0f * cosf(finaleProgress * M_PI * 3.0f);  // Right leg jumping
            }
        }
    }
    ...
    // Check if we should show the ice cream cone instead of the main character
    if (showIceCreamCone) {
        // BOMB EXPLOSION SOUND EFFECT - Play loud sound when ice cream first appears
        if (!lastIceCreamState && !bombSoundPlayed) {
            // Ice cream just became visible - play loud bomb explosion sound!
            playLoudBombExplosion();
            bombSoundPlayed = true; // Mark sound as played for this session
        }
        ...
        // Reset bomb sound when ice cream is hidden so it can play again next time
        if (lastIceCreamState) {
            bombSoundPlayed = false; // Reset for next time ice cream is shown
        }
        ...
        // Apply dance body rotation if animation is active
        if (isDancing) {
            // Apply rotation at the base of the upper body (waist)
            glTranslatef(0.0f, 1.2f, 0.0f);  // Move to pivot point (waist)
            glRotatef(danceBodyAngle, 0.0f, 1.0f, 0.0f);  // Rotate around Y-axis (body rotation)
            glTranslatef(0.0f, -1.2f, 0.0f);  // Move back
        }

        // Apply upper body rotation if enabled
        if (isUpperBodyRotating) {
            // Move to the center of upper body for proper rotation
            glTranslatef(0.0f, 2.8f, 0.0f);  // Move to upper body center
            glRotatef(upperBodyRotationY, 0.0f, 1.0f, 0.0f);  // Rotate around Y-axis
            glTranslatef(0.0f, -2.8f, 0.0f);  // Move back
        }
    ...
    }  // End else block for main character drawing
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE, LPSTR, int nCmdShow)
{
    ...
    while (true)
    {
        ...
        // Update ice cream state tracking
        lastIceCreamState = showIceCreamCone;
    ...
    // Clean up forest texture before exiting
    if (forestTexture != 0) {
        glDeleteTextures(1, &forestTexture);
        forestTexture = 0;
    }

    // Clean up hill texture before exiting
    if (hillTexture != 0) {
...
void drawSimpleLeg(bool isLeft) {
    ...
    // Apply kick animation if active (only for right leg)
        ...
        // Apply dance animation for both legs
        glRotatef(danceLegAngle[legIndex], 1, 0, 0);  // Both legs move during dance
    }
    else {
        // Normal walking animation
